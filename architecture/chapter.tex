...
\Figureref{architecture}{rss-cloud} shows how $s_i$ relates to $p_i$ for a single location.
...
The function can be understood by realising that for $d_i > 0dB$, large and very large penalties are given.
...
Considering the changes in behaviour between these versions, as shown in \figureref{architecture}{rss-cloud}, optimising for other listening intervals will probably lead to other parameters.
...
\fig{\gnuplot{architecture}{brp-o-parameter-effect}{Positioning errors using SSD-O}}
As expected, \aBRP-O does not give the same performance benefits as SSD-O, however still performs better than \aBRP.

As \figureref{architecture}{brp-o-parameter-effect} shows, \aBRP-O works best with a 
