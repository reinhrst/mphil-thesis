...
This highly fluctuating RSS value is unhelpful for most positioning methods.
A single value for the RSS is preferred, with the most important thing not being the absolute RSS value, but the property that the RSS decreases with distance, ideally with 6dB when the distance doubles.
...
The same channel in both runs have a Pearson correlation of between 0.83 and 0.88, with a p-value of 0.000, considerably less than the correlation between different runs with the receiver in the same spot, but more than that between different channels in the same run.
...
Again we see some similarities, but also a lot of differences, with correlations between 0.48 and 0.66, the same as between different channels in one run.
...
One area in which I believe one could take advantage of \mpi, is in motion detection, although it may be of limited use.
Since small movements result in large RSS changes, a device can detect if it moves.
However since small changed in the environment influence the position of these drops (see also \sectionref{rss}{busyroom}, a changing RSS should probably be interpreted as movement in general, not necessarily of the device itself.
If one were to attempt this, it were to help greatly if packages from different channels could be observed, either because the receiving device exposes these, or because beacons are known to only advertise certain content on certain channels\footnote{It should be noted that this is against the Bluetooth 4.0 and 4.1 specification; I do not believe any application would break if one were to transmit different messages on different channels.}.
...
Next I repeated the experiment with first a single beacon set to transmit at 10Hz (as discussed previously, the actual transmit frequency of these beacons is 9.5Hz, this has been taken into account in the calculations in this section), then multiple beacons, all set to 10Hz.
...
\Tableref{rss}{packet-loss} shows the chance of not observing a beacon that is present, if there is a random 38\% chance that a packet is lost, at different listening intervals, where listening intervals are measured in beacon intervals: a listening interval of 3 means three times the time between 2 advertising packets from a single beacon, or 0.3 seconds if the beacon broadcasts at 10Hz.
\Tableref{rss}{packet-loss-empirical} shows in which percentage of the measured cases we failed to observe all beacons; it shows that although the packet loss with 20 beacons is the same 38\%, the chances of missing at least one beacon are much higher.
This indicates that not each package has the same chance of being missed (something that we also see in \figureref{rss}{busyroom}, where packages from some beacons are lost more often than others), even though all beacons were spread around the \device, with a clear line-of-sight, all within 20 centimetres.
