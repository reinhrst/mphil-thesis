...
As an example, even though the beacons are set to advertise at 10Hz, in \figureref{rss}{busyroom} we never see an average of more than 6 packets (and often less).
...
\Tableref{rss}{packet-loss} shows that with one beacon, if we wait for four transmission intervals, there is still a 2.6\% chance we did not receive it.
With 20 beacons, after four transmission intervals we have a 40\% chance that not all beacons were received at least once.
Even if all beacons are received once, calculating the median or maximum RSS as suggested in \sectionref{rss}{mpi} would require packets from multiple channels for each beacon.
